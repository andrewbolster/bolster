...
HESA (the Higher Education Statistics Agency), covering two related
populations:

- **NI-domiciled students enrolled at any UK HEI** (Table 1) -- by level of
  study, mode of study and location of institution (NI / GB / Open
  University).
- **All enrolments at NI's own HEIs** (Table 8), regardless of domicile --
  by level of study, mode of study and country of domicile (NI / GB / RoI /
  other EU / non-EU).

The publication has 14 tables in total (with dozens of lettered
percentage-change and demographic sub-tables); this module covers the two
headline cross-tabulations. Both share the same layout: a title row, a
three-row nested column header (level of study -> location/domicile ->
NI/GB/.../Total), then repeating ``Mode`` marker rows (Full-time, Part-time,
Total) each followed by one data row per academic year.

Data Source:
    **Hub page**: https://www.economy-ni.gov.uk/articles/higher-education-enrolments

    The current edition is discovered by scraping the hub page for the
    "Enrolments at UK Higher Education Institutions" publication link, then
    scraping that publication page for its workbook. Filenames change every
    year (e.g. ``Enrolments 2024_25 Tables.xlsx``), so the URL is always
...
Example:
    ...
    >>> df = enrolments.get_ni_domiciled_enrolments()  # doctest: +SKIP
...
HUB_URL = "https://www.economy-ni.gov.uk/articles/higher-education-enrolments"
_PUB_TEXT = "Enrolments at UK Higher Education Institutions"

# Annual publication -- re-check monthly for a new edition.
_CACHE_TTL_HOURS = 24 * 30

_downloader = CachedDownloader("dfe_higher_education_enrolments", timeout=60)
...
def get_workbook_url(force_refresh: bool = False) -> str:
    """Find the URL of the latest Enrolments workbook.
    ...
    try:
        return find_publication_link(
            ...
            file_href_contains="Enrolments",
            force_refresh=force_refresh,
        )
    except Exception as e:
        raise DfEDataNotFoundError(f"Could not find the Enrolments workbook: {e}") from e


def _parse_wide_table(path, sheet_name: str) -> pd.DataFrame:
    """Parse a "Mode and Year" cross-tabulation sheet (Table 1 / Table 8 shape).
    ...
    Returns:
        ...
        (level of study x location/domicile) combination.

    Raises:
        DfEDataNotFoundError: If the sheet has no "Mode and Year" header row.
    """
...
def get_ni_domiciled_enrolments(force_refresh: bool = False) -> pd.DataFrame:
    """Return NI-domiciled students enrolled at any UK HEI (Table 1).
    ...
    Returns:
        ...
        ``total``), ``year`` (e.g. ``"2024/25"``), one number column per
        (level of study x location) combination -- e.g.
        ``first_degree_ni``, ``postgraduate_total`` -- and
        ``ni_heis_incl_ni_ou``, a convenience total across all levels for
        students at NI institutions specifically.

    Example:
        >>> df = get_ni_domiciled_enrolments()  # doctest: +SKIP
...
def get_ni_hei_enrolments(force_refresh: bool = False) -> pd.DataFrame:
    """Return all enrolments at NI's own HEIs, by domicile (Table 8).
    ...
    Returns:
        ...
        (level of study x country of domicile) combination -- e.g.
        ``first_degree_ni``, ``postgraduate_non_eu``.

    Example:
        >>> df = get_ni_hei_enrolments()  # doctest: +SKIP
        >>> sorted(df['mode'].unique())  # doctest: +SKIP
        ['full_time', 'part_time', 'total']
    """
    path = download_file(get_workbook_url(force_refresh=force_refresh), force_refresh=force_refresh)
    return _parse_wide_table(path, "Table8")
...
def validate_data(df: pd.DataFrame) -> bool:
    ...
    Args:
        df: DataFrame returned by :func:`get_ni_domiciled_enrolments` or
            :func:`get_ni_hei_enrolments`.
    ...
    Example:
        >>> validate_data(get_ni_domiciled_enrolments())  # doctest: +SKIP
        True
    """
    if df.empty:
