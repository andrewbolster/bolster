...
from .data_sources.dfe import higher_education_qualifications as dfe_higher_education_qualifications
...
@dfe.command(name="qualifications")
@click.option(
    "--topic",
    type=click.Choice(["ni-domiciled", "ni-heis"], case_sensitive=False),
    default="ni-domiciled",
    show_default=True,
    help="ni-domiciled: NI-domiciled students gaining qualifications at any UK HEI. "
    "ni-heis: all qualifications gained at NI's own HEIs.",
)
@click.option(
    "--format",
    "output_format",
...
@click.option("--force-refresh", is_flag=True, help="Force re-download even if cached")
@click.option("--save", help="Save data to file (specify filename)")
def dfe_qualifications_cmd(topic, output_format, force_refresh, save):
    r"""Qualifications gained at UK Higher Education Institutions: NI Analysis.

    \b
    Annual DfE bulletin, sourced from HESA -- the direct companion to
    `bolster dfe enrolments`: NI-domiciled students gaining qualifications
    at any UK HEI (by level, mode and location), and all qualifications
    gained at NI's own HEIs regardless of domicile (by level, mode and
    country of domicile). Academic years 2015/16 to present.
    ...
    Examples:
        ...
        All qualifications gained at NI's own HEIs, by domicile::

            bolster dfe qualifications --topic ni-heis

        Save as JSON::

            bolster dfe qualifications --topic ni-heis --format json --save ni_heis.json

    Source:
        https://www.economy-ni.gov.uk/articles/higher-education-qualifications
    """
    console = Console()

    try:
        with console.status(f"[bold green]Downloading qualifications data ({topic})..."):
            if topic == "ni-domiciled":
                data = dfe_higher_education_qualifications.get_ni_domiciled_qualifications(force_refresh=force_refresh)
            else:
                data = dfe_higher_education_qualifications.get_ni_hei_qualifications(force_refresh=force_refresh)

        console.print("[green]Qualifications data retrieved successfully[/green]")
        console.print(f"[cyan]Topic: {topic} | Rows: {len(data)}[/cyan]")
