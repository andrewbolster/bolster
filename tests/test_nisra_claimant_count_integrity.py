...
class TestLGDIntegrity:
    ...
    def test_date_is_datetime(self, latest_data: pd.DataFrame) -> None:
        assert pd.api.types.is_datetime64_any_dtype(latest_data["date"]), "date column should be datetime type"
    ...
    def test_eleven_districts_in_latest_month(self, latest_data: pd.DataFrame) -> None:
        ...
        assert len(districts) == 11, f"Expected 11 districts, got {len(districts)}: {list(districts['geography'])}"
    ...
    def test_claimant_totals_positive(self, latest_data: pd.DataFrame) -> None:
        assert (latest_data["claimants_total"] > 0).all(), "All total claimant counts should be positive"

    def test_rates_in_valid_range(self, latest_data: pd.DataFrame) -> None:
        rates = latest_data["claimant_rate_total_pct"].dropna()
        assert (rates >= 0).all() and (rates <= 100).all(), f"Rates out of range: min={rates.min()}, max={rates.max()}"
    ...
    def test_historical_coverage_from_2005(self, latest_data: pd.DataFrame) -> None:
        ...
        assert min_date.year <= 2005, f"Expected data from 2005, earliest is {min_date}"
    ...
    def test_belfast_highest_claimants_recent(self, latest_data: pd.DataFrame) -> None:
        ...
        assert max_row["geography"] == "Belfast", f"Expected Belfast to have most claimants, got {max_row['geography']}"
...
class TestAAIntegrity:
    ...
    def test_eighteen_assembly_areas_in_latest_month(self, latest_data: pd.DataFrame) -> None:
        ...
        assert len(areas) == 18, f"Expected 18 Assembly Areas, got {len(areas)}: {list(areas['geography'])}"
