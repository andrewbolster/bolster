...
class TestDataIntegrity:
    ...
    def test_reasonable_population_totals(self, projections_data):
        ...
        totals = projections_data[projections_data["sex"] == "All persons"].groupby("year")["population"].sum()
...
class TestValidation:
    ...
    def test_validate_missing_columns(self):
        with pytest.raises(NISRAValidationError, match="Missing required columns"):
            population_projections.validate_projections(pd.DataFrame({"year": [2030], "population": [1000]}))
